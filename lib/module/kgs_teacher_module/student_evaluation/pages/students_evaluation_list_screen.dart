...
import '../widgets/student_evaluation_list_tile.dart';
      ...
      create:
          (context) =>
              StudentsEvaluationCubit(sl())
                ..init(input: input, studentList: studentList),
...
class StudentsEvaluationListScreenView extends StatefulWidget {
  const StudentsEvaluationListScreenView({super.key});
      ...
      body: BlocBuilder<StudentsEvaluationCubit, StudentsEvaluationState>(
        builder: (context, state) {
          return RefreshIndicator(
            onRefresh: () async {
              await stdContext.fetchStudentsList(
                stdContext.input!,
                showLoading: false,
              );
            },
                  ...
                  return StudentEvaluationListTile(
                    ...
                    onProcessClick: () {
                      if (student.isProcessed == true) {
                        UnProcessResultInput input = UnProcessResultInput(
                        ...
                        stdContext.processResult(input).then((v) async {
                          ...
                          if (student.isProcessed == isProcessed) {
                            DisplayUtils.showToast(
                              context,
                              "Please Submit Evaluation Form First",
                            );
                          }
                        });
                      }
                    ...
                    onTap: () async {
                      if (student.isProcessed == true) {
                        DisplayUtils.showToast(
                          context,
                          "Un Process Result to make Changes",
                        );
                        return;
                      }
                      ...
                      StudentOutcomesListResponse? response = await stdContext
                          .fetchStudentOutcomes(input);
                      if (response != null) {
